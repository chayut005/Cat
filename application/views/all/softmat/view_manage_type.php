                ...
                <div class="col-sm-12">
                    ...
                    <div class="nav-align-top mb-4">
                        <ul class="nav nav-pills mb-3" role="tablist">
                            ...
                            <li class="nav-item">
                                <button type="button" class="nav-link" role="tab" data-bs-toggle="tab" data-bs-target="#navs-top-Type" aria-controls="navs-top-Type" aria-selected="false">
                                    Type
                                </button>
                            </li>
                        </ul>
                                ...
                                <div class="row">
                                    <div style="margin-bottom: 10px;" class="col-lg-12 col-sm-12">
                                        ...
                                        <div style="float: right;" class=" input-group-outline" style="margin-bottom:1rem">
                                            ...
                                            <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#modal_use_type">
...
<!-- ------------------------------------------------------------------------------------------------------------------------ -->
<div class="modal fade" id="modal_use_type" aria-hidden="true" data-bs-backdrop="static" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title" id="modalCenterTitle">SELECT TYPE</h6>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <table id="data_insert_type" class="table table-striped display nowrap" style="width:100%; text-align:center; font-size:12px;">
                    <thead>
                        <tr>
                            <th style="text-align:center;">No</th>
                            <th style="text-align:center;">Type</th>
...
<script>
    ...
    function button_re_type(type_id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // alert(user_id)
                $.ajax({
                    url: '<?php echo base_url(); ?>set_time/re_type',
                    type: "POST",
                    dataType: 'json',
                    data: {
                        type_id: type_id
    ...
    function button_delete_type(type_id) {
        event.preventDefault();
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // alert(user_id)
                $.ajax({
                    url: '<?php echo base_url(); ?>set_time/delete_type',
                    type: "POST",
                    dataType: 'json',
                    data: {
                        type_id: type_id
    ...
    function button_enable_type(type_id) {
        event.preventDefault();
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?php echo base_url(); ?>set_time/enable_type',
                    type: "POST",
                    dataType: 'json',
                    data: {
                        type_id: type_id
    ...
    function button_disable_type(type_id) {
        event.preventDefault();
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?php echo base_url(); ?>set_time/disable_type',
                    type: "POST",
                    dataType: 'json',
                    data: {
                        type_id: type_id
    ...
    function edit_type_main(type_id) {
        alert(type_id)
    }
        ...
        $.ajax({
            ...
            success: function(re_data) {
                ...
                $.each(re_data, function(key_re, val_re) {
                    ...
                    html_re += '<div>'
                    html_re += '<img style="    object-fit: cover;display: block;margin: auto;height: 180px;width: 180px;border-radius: 50%;" src="https://www.bangkokbanksme.com/uploads/topics/15893439077328.jpg" alt=""/>'
                    html_re += '</div>'
                    html_re += '<div class="content">'
                    html_re += '<h3 class="title">' + val_re.type_name + '</h3><span><span class="spinner-grow text-success" style="height:13px; width:13px;     animation: 1.45s linear infinite spinner-grow;" role="status"><span class="visually-hidden">Loading...</span></span></span>'
                    // html_re += '<p>' + val_re.date_e + ' Day ' + val_re.h_e + ' hour ' + val_re.m_e + ' minute ' + '</p>'
                    html_re += '</div>'
                    html_re += '<div class="button-wrapper">'
                    html_re += '<a onclick="button_delete_type_dep(' + val_re.dep_id + ',' + val_re.type_id + ')" href="" style="    display: flex;justify-content: center; align-items: center;" type="button" onclick="" class="btn btn-sm btn-danger ">'
    ...
    $(document).ready(function() {
        ...
        var table = $('#main_type').DataTable({
            ...
            scrollX: true,
            ajax: {
                url: '<?php echo base_url(); ?>GET_API/data_main_type',
                type: 'post',
                dataType: 'json',
                // data: function(data) {
                //     data.start_date = $('#start_date').val();
                    ...
                    "render": function(data, type, row, meta) {
                        if (type === 'display') {
                            if (row.status_type === '1') {
                                data = '<div class="form-check form-switch mb-2"><input onclick="button_disable_type(' + data + ')"  class="form-check-input" type="checkbox" checked id="flexSwitchCheckDefault"></div>'
                            } else if (row.status_type === '0') {
                                data = '<div class="form-check form-switch mb-2"><input onclick="button_enable_type(' + data + ')"  class="form-check-input" type="checkbox" id="flexSwitchCheckDefault"></div>'
                            }
                        }
                        return data;
                    }
    ...
    $(document).ready(function() {
        $.fn.DataTable.ext.pager.numbers_length = 5;
        var cnt = 1;
        var table = $('#data_insert_type').DataTable({
            // lengthChange: false,
            // buttons: ['copy', 'excel', 'pdf', 'colvis'],form_update_time_pri data_priority_check_add
            lengthMenu: [
                [10, 25, 50, 100],
            ...
            scrollX: true,
            ajax: {
                url: '<?php echo base_url(); ?>GET_API/data_type_table',
                type: 'post',
                dataType: 'json',
                // data: function(data) {
                //     data.start_date = $('#start_date').val();
