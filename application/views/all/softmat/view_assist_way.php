    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $.each(data, function(key, val) {
          edit_dep_issue_way = ''
          edit_dep_issue_way = '<option selected value="' + val.dep_issue_id + '">' + val.dep_issue + '</option>'
          ...
          $.ajax({
            ...
            success: function(department_data) {
              ...
              $.each(department_data, function(key_dep, val_dep) {
                ...
                html_support_dep_way += '<option  ' + html_support_dep_way_check + '  value="' + val_dep.dep_id + '">' + val_dep.dep_name + '</option>'
            ...
            success: function(reply_support) {
              ...
              if (reply_support !== null && reply_support !== '') {
                ...
                $.each(reply_support, function(key_support, val_support) {
                  ...
                  html_support_by_way += '<option ' + html_check_way + '  value="' + val_support.user_id + '">' + val_support.f_name + ' ' + val_support.l_name + ' (' + val_support.employee + ')</option>'
                  html_check_way = '';
                })
              } else {
            ...
            success: function(reply_type) {
              ...
              if (reply_type !== null && reply_type !== '') {
                ...
                $.each(reply_type, function(key_type, val_type) {
                  ...
                  html_type_way += '<option  ' + html_type_way_check + '  value="' + val_type.type_id + '">' + val_type.type_name + '</option>'
                  html_type_way_check = ''
                })
              } else {
            ...
            success: function(reply_sys) {
              ...
              if (reply_sys !== null && reply_sys !== '') {
                ...
                $.each(reply_sys, function(key_sys, val_sys) {
                  ...
                  html_system_way += '<option  ' + html_type_way_check + '  value="' + val_sys.system_id + '">' + val_sys.system_name + '</option>'
                })
              } else {
    ...
    $.ajax({
      ...
      success: function(reply_support) {
        ...
        if (reply_support !== null && reply_support !== '') {
          ...
          html_support_by_way += '<option selected value="">--- Support ---</option>';
          $.each(reply_support, function(key_support, val_support) {
            html_support_by_way += '<option   value="' + val_support.user_id + '">' + val_support.f_name + ' ' + val_support.l_name + ' (' + val_support.employee + ')</option>'
          })
        } else {
          $("#edit_html_support_by_way").attr("disabled", true)
          html_support_by_way = '';
        ...
        success: function(reply_check) {
          if (reply_check !== null && reply_check !== '') {
            ...
            $.each(reply_check, function(key_pri, val_pri) {
              html_priority_way += '<option  value="' + val_pri.pri_id + ' ' + val_pri.time_priority + '">' + val_pri.pri_name + '</option>'
            })
      ...
      $.ajax({
        ...
        success: function(reply_sys) {
          ...
          if (reply_sys !== null && reply_sys !== '') {
            ...
            $.each(reply_sys, function(key_sys, val_sys) {
              // alert(val_sys.system_id)
              html_system_way += '<option  value="' + val_sys.system_id + '">' + val_sys.system_name + '</option>'
            })
          } else {
            $("#html_system_way").attr("disabled", true)
            html_system_way = '';
          ...
          "render": function(data, type, row, meta) {
            if (type === 'display') {
              if (row.status_qu === '1') {
                ...
                data = '<div style="background-color:#6600bf;border-radius:10px; color:#ffffff;  ">Cancel</div>'
              } else {
