	...
	<div class="modal-dialog modal-dialog-centered modal-xl">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="" id="form_quest_way_modal" class="was-validated">
					<div class="row">
...
<script>
		...
		$.ajax({
			...
			success: function (data) {
				...
				$.each(data, function (key, val) {
					...
					edit_issue_by_way = ''
					edit_issue_by_way = '<option selected value="' + val.issue_by_id + '">' + val
						.issue_by + '</option>'
					...
					$.ajax({
						...
						success: function (department_data) {
							...
							$.each(department_data, function (key_dep, val_dep) {
								...
								html_support_dep_way += '<option  ' +
									html_support_dep_way_check + '  value="' +
									val_dep.dep_id + '">' + val_dep.dep_name +
									'</option>'
						...
						success: function (reply_support) {
							...
							if (reply_support !== null && reply_support !== '') {
								...
								$.each(reply_support, function (key_support, val_support) {
									...
									html_support_by_way += '<option ' +
										html_check_way + '  value="' + val_support
										.user_id + '">' + val_support.f_name +
										' ' + val_support.l_name + ' (' +
								...
								html_support_by_way = '';
								html_support_by_way +=
									'<option selected value="">--- (No Employee) ---</option>';
							}
						...
						success: function (reply_type) {
							...
							if (reply_type !== null && reply_type !== '') {
								...
								$.each(reply_type, function (key_type, val_type) {
									...
									html_type_way += '<option  ' +
										html_type_way_check + '  value="' +
										val_type
										.type_id + '">' + val_type.type_name +
										'</option>'
									html_type_way_check = ''
								})
							} else {
						...
						success: function (reply_sys) {
							...
							if (reply_sys !== null && reply_sys !== '') {
								...
								$.each(reply_sys, function (key_sys, val_sys) {
									...
									html_system_way += '<option  ' +
										html_type_way_check + '  value="' + val_sys
										.system_id + '">' + val_sys.system_name +
										'</option>'
								})
							} else {
								...
								html_system_way = '';
								html_system_way +=
									'<option selected value="">--- (No System) ---</option>';
							}
	...
	function send_data_quest_way_modal() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				const form = document.getElementById('form_quest_way_modal');
				var form_data = new FormData(form);
				$.ajax({
					type: 'POST',
					dataType: 'json',
					url: '<?php echo base_url(); ?>request/send_data_quest_way_modal',
					data: form_data,
					cache: false,
					processData: false,
					contentType: false,
				...
				$.ajax({
					...
					success: function (reply_cancel) {
						...
						if (reply_cancel['reply'] !== true && reply_cancel['reply'] !== false) {
							...
							Swal.fire({
								html: "<p>" + reply_cancel['html'] + "</p><p>" +
									reply_cancel['html_eng'] + "</p>",
								icon: 'success',
		...
		if (input_img.files && input_img.files[0]) {
			...
			reader.onload = function (e) {
				...
				if (chk[0] != "application") {
					...
					$("#show_data_way1").attr("src",
						"<?php echo base_url(); ?>./themes/softmat/img/upload_file_icon.png")
				}
			}
