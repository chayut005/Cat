...
<script>
    ...
    function clear_data_re() {
        ...
        html_issue_by += '<select onchange="issue_by(value)" disabled id="html_issue_by" name="re_issue" class="form-control" required>'
        ...
        if (dep_sup_id !== '') {
            $.ajax({
                ...
                success: function(reply_check) {
                    // console.log(reply_check)
                    if (reply_check !== null && reply_check !== '') {
                        $("#col_priority_type").removeClass("col-lg-12 col-mb-12 col-sm-12").addClass("col-lg-6 col-mb-6 col-sm-6");
                        html_priority = '';
                        html_priority += '<div class=" input-group-outline input-group-sm">';
                        html_priority += '<label>Priority:</label>';
                        html_priority += '<select id="html_priority" name="re_priority" class="form-control" required>';
                        html_priority += '<option selected value="">--- Priority ---</option>';
                        $.each(reply_check, function(key_pri, val_pri) {
                            html_priority += '<option  value="' + val_pri.pri_id + ' ' + val_pri.time_priority + '">' + val_pri.pri_name + '</option>'
                        })
    ...
    function support_by(user_id) {
        ...
        if (user_id !== '') {
            $.ajax({
                ...
                beforeSend: function() {
                    $("#img_support_name").attr("src", "<?php echo base_url(); ?>./themes/softmat/img/loading3.gif")
                ...
                success: function(data_user) {
                    $.each(data_user, function(key_user, val_user) {
                        $('#img_support_name').attr('src', '<?php echo base_url(); ?>' + val_user.path_img_user)
    ...
    function issue_by(user_id) {
        ...
        if (user_id !== '') {
            $.ajax({
                ...
                beforeSend: function() {
                    $("#img_issue_name").attr("src", "<?php echo base_url(); ?>./themes/softmat/img/loading3.gif")
                ...
                success: function(data_user) {
                    $.each(data_user, function(key_user, val_user) {
                        $('#img_issue_name').attr('src', '<?php echo base_url(); ?>' + val_user.path_img_user)
                ...
                success: function(reply_support) {
                    ...
                    if (reply_support !== null && reply_support !== '') {
                        ...
                        $.each(reply_support, function(key_support, val_support) {
                            html_support_by += '<option  value="' + val_support.user_id + '">' + val_support.f_name + ' ' + val_support.l_name + ' (' + val_support.employee + ')</option>'
                        })
                    } else {
                ...
                success: function(reply_type) {
                    ...
                    if (reply_type !== null && reply_type !== '') {
                        ...
                        $.each(reply_type, function(key_type, val_type) {
                            html_type += '<option  value="' + val_type.type_id + '">' + val_type.type_name + '</option>'
                        })
                    } else {
                ...
                success: function(reply_sys) {
                    ...
                    if (reply_sys !== null && reply_sys !== '') {
                        ...
                        $.each(reply_sys, function(key_sys, val_sys) {
                            ...
                            html_system += '<option  value="' + val_sys.system_id + '">' + val_sys.system_name + '</option>'
                        })
                    } else {
    ...
    function text_box() {
        ...
        html_issue_by += '<input type="text" class="form-control" placeholder="Enter Issue name" name="issue_box" required>'
        html_issue_by += '<button class="btn  btn-sm btn-secondary" onclick="open_issue(' + $('select[name=re_department_issue]').val() + ')" type="button"><i class="bx bx-carousel"></i></button>'
        html_issue_by += '</div>'
        $("#html_issue_by_box").html(html_issue_by)
        $('#issue_name').html('<span>xxxxx</span>')
        $('#img_issue_name').attr('src', '<?php echo base_url(); ?>./themes/softmat/img/user.png')
            ...
            $.ajax({
                ...
                success: function(reply_issue) {
                    ...
                    if (reply_issue !== null && reply_issue !== '') {
                        ...
                        html_issue_by += '<label>Issue By:</label>'
                        html_issue_by += '<div class=" input-group-outline input-group  input-group-sm">'
                        html_issue_by += '<select onchange="issue_by(value)" id="html_issue_by" name="re_issue" class="form-control" required>'
                        html_issue_by += '<option selected value="">--- Issue ---</option>'

                        // html_issue_by += '<option selected value="">--- Issue ---</option>';
                        $.each(reply_issue, function(key_issue, val_issue) {
                            html_issue_by += '<option  value="' + val_issue.user_id + '">' + val_issue.f_name + ' ' + val_issue.l_name + ' (' + val_issue.employee + ')</option>'
                        })
                        html_issue_by += '</select>'
                        html_issue_by += '<button class="btn  btn-sm btn-secondary" onclick="text_box()" type="button"><i class="bx bx-text"></i></button>'
                        html_issue_by += '</div>'
                    } else {
        ...
        $.ajax({
            ...
            success: function(department_data) {
                ...
                $.each(department_data, function(key_dep, val_dep) {
                    html_issue_dep += '<option  value="' + val_dep.dep_id + '">' + val_dep.dep_name + '</option>'
                })
